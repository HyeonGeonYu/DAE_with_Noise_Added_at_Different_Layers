...
SAVE_PATH = "saved_models/autoencoder_2023_07_30_16_31_45_11"
test_scheme = "dnn"
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
criterion = torch.nn.MSELoss().to(device)

net = torch.load(SAVE_PATH).to(device)
noise_location_idx = 11
SNR = 12
std = (2/10**SNR)**0.5
...
with torch.no_grad():
    ...
    plt.colorbar()
    #plt.show()
    plt.savefig(SAVE_PATH+"_manifold_idx_"+str(noise_location_idx))
    plt.close('all')
    ...
    for idx, (x_batch, target) in enumerate(test_data):
        ...
        # plt.show()
        plt.savefig(SAVE_PATH+"_"+"result_idx_"+str(noise_location_idx))
        break
    plt.close('all')
